...
const tabLabel = (intl, tab) => {
  let key = null;
  if (tab === GENERAL) {
    key = 'EditTeacherWizard.tabLabelGeneral';
  } else if (tab === LOCATION) {
    key = 'EditTeacherWizard.tabLabelLocation';
  } else if (tab === PRICING) {
...
const tabCompleted = (tab, listing) => {
  ...
  switch (tab) {
    case GENERAL:
      return !!(description && title && publicData.subjects && publicData.levels && publicData.hours && publicData.numberOfHours);
    case LOCATION:
...
const createReturnURL = (returnURLType, rootURL, routes, pathParams) => {
  const path = createResourceLocatorString(
    'EditTeachingStripeOnboardingPage',
    routes,
...
class EditTeacherWizard extends Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    if (!tabsStatus[selectedTab]) {
      ...
      return <NamedRedirect name="EditTeacherPage" params={{ ...params, tab: nearestActiveTab }} />;
    }
    ...
    const tabLink = tab => {
      return { name: 'EditTeacherPage', params: { ...params, tab } };
    };
    ...
    if (returnedNormallyFromStripe && stripeConnected && !requirementsMissing) {
      return <NamedRedirect name="EditTeacherPage" params={pathParams} />;
    }
...
EditTeacherWizard.defaultProps = {
  className: null,
  currentUser: null,
  rootClassName: null,
  listing: null,
